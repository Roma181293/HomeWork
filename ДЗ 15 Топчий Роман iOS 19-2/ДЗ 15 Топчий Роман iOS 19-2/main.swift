...
class Mother {
    ...
    lazy var childrenGoToSleep_closure: ()->() =  {[children] in
        for child in children {
            child.goToSleep()
        }
    }
    lazy var husbandBuyBoots_closure: ()->() =  {[weak husband] in
        guard let husband = husband else {return}
        husband.buyBoots()
    }
...
class Father {
    ...
    lazy var wifeCookMeat_closure: ()->() =  {[weak wife] in
        guard let wife = wife else {return}
        wife.cookMeat()
    }
...
class Child {
    ...
    lazy var letsPlayFootbal_closure: ()->() =  {[weak self] in
        guard let `self` = self else {return}
        guard let mother = self.mother else {return}
        print("-\(self.name): Lets play football")
        for index in 0..<mother.children.count {
            if !(mother.children[index] === self) {
                mother.children[index].playFootbal(name: self.name)
            }
        }
    }
    lazy var momCookMeat_closure: ()->() =  {[weak self] in
        guard let `self` = self else {return}
        print("-\(self.name): Mom, please cook a meat")
        guard let mother = self.mother else {return}
        mother.cookMeat()
...
family.mother?.childrenGoToSleep_closure()
family.father?.childrenGoToSleep_closure()

family.father?.wifeCookMeat_closure()
family.mother?.husbandBuyBoots_closure()

family.children[0].momCookMeat_closure()
family.children[1].momCookMeat_closure()

for i in 0..<family.children.count {
    family.children[i].letsPlayFootbal_closure()
}
family = nil
